...
INSERT INTO "MEMBER"
VALUES('MEM03', '1Q2W3E4R', '박철수', '970101-1385478', DEFAULT);
...
VALUES
	('MEM04', 'QQQQ2222', '신짱구', '990304-1122334');
...
--	외부로 테이블의 값을 저장할 수 있다(== 다른 테이블에서 사용할 수 있다(참조))
...
CREATE TABLE USER_GRADE(
  GRADE_CODE NUMBER PRIMARY KEY,	-- FK 제약조건으로 참조할 수 있음
...
CREATE TABLE USER_USED_FK(
  ...
  -- USER_GRADE의 테이블의 GRADE_CODE 컬럼에 작성된 값만 가능
  -- == 참조
...
WHERE GRADE_CODE = 10;
-- 무결성 제약조건(KH10_PMW.GRADE_CODE_FK1)이 위배되었습니다
-- 자식 레코드가 발견되었습니다.
...
FROM
	USER_USED_FK CHILD
LEFT JOIN
	USER_GRADE PARENT ON (CHILD.GRADE_CODE = PARENT.GRADE_CODE);
...
/* CHECK 제약조건은 설정이 간편하지만 조건을 수정하는데 어려움이 있음
 * -> 그래서 조건이 다양한 경우에는 FK 제약조건을 이용하는 것이 좋음
 */
...
CREATE TABLE USER_TEST(
	...
	CONSTRAINT CK_STATUS CHECK(GENDER IN ('Y','N'))
);
----------------------------------------------------------------------------------------------------------------
...
ALTER TABLE LOCATION ADD CONSTRAINT NATIONAL_CODE_FK
