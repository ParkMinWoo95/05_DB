...
-- DEPARTMAENT 테이블을 복사한 DEPT_COPY2 테이블 생성
...
-- 시퀀스 SEQ_TEST_NO.CURRVAL은 이 세션에서는
-- 정의 되어 있지 않습니다
-- == NEXTVAL가 한 번도 호출 된적이 없다

SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL; -- 100
SELECT SEQ_TEST_NO.CURRVAL FROM DUAL; -- 100(오류없음)

SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;

-- TB_TEST 테이블에 INSERT 시 PK 컬럼 값을 시퀀스로 얻어와서 삽입
INSERT INTO TB_TEST VALUES(SEQ_TEST_NO.NEXTVAL, '짱구'); -- 125
INSERT INTO TB_TEST VALUES(SEQ_TEST_NO.NEXTVAL, '철수'); -- 130
INSERT INTO TB_TEST VALUES(SEQ_TEST_NO.NEXTVAL, '유리'); -- 135
...
/* 인덱스를 활용해서 검색하는 방법
 * - SELECT문의 WHERE절에 INDEX가 설정된 컬럼을 언급
 */
...
-- TEST_NO(PK, INDEX), TESTID(NOT NULL)
...
/* INDEX는 데이터의 양, 쿼리의 복잡도에 따라서 속도가 더 증가할 수 있다 */
