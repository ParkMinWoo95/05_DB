...
/* GROUP BY가 사용된 SELECT문의 SELECT절에는 그룹 함수 또는
 * GROUP BY에 사용된 컬럼명만 작성 가능
 */
...
SELECT
	...
	J.JOB_NAME,
	CASE 
		-- 현재 행의 EMP_ID와 일치하는 값이 서브쿼리 결과에
		-- 있으면 '사수'
		-- 없으면 '사원'
		WHEN E.EMP_ID IN (
...
-- 비교하려는 컬럼을 ()로 묶어서 여러 컬럼을 한 번에 비교
...
SELECT
	E.EMP_NO, E.EMP_NAME, E.DEPT_CODE, E.JOB_CODE, D.DEPT_TITLE, J.JOB_NAME
FROM
	EMPLOYEE E
JOIN
	DEPARTMENT D ON (D.DEPT_ID = E.DEPT_CODE)
...
--    사번, 이름, 부서코드, 직급코드, 고용일
SELECT
	E.EMP_ID, E.EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM 
	EMPLOYEE E
WHERE 
	(E.DEPT_CODE, E.JOB_CODE) = (
		SELECT DEPT_CODE, JOB_CODE
		FROM EMPLOYEE
		WHERE	TO_CHAR(HIRE_DATE, 'YYYY')LIKE '2010%'
		);

-- 3. 87년생 여자 사원과 동일한 부서이면서 동일한 사수를 가지고 있는 사원을 조회하시오
--    사번, 이름, 부서코드, 사수번호, 주민번호, 고용일     
SELECT
	E.EMP_NO, E.EMP_NAME, E.DEPT_CODE, 
FROM
	EMPLOYEE E;
...
-- 다중행 서브쿼리 -> IN, > ANY, > ALL 연산자 이용
-- 다중열 서브쿼리 -> WHERE절 컬럼을 () 여러개 사용
 ...
 * 3) 서브쿼리의 결과가 먼저 해석된 메인 쿼리 1행에 영향을 줌
 * 		-> 최종 결과(RESULT SET)에 포함 될ㄷ지 안될지를 지정
 */
...
SELECT 
	MAIN.EMP_ID, MAIN.EMP_NAME, NVL(D.DEPT_TITLE, '없음') AS DEPT_TITLE,
...
-- (스칼라 + 상관 쿼리)
SELECT 
	MAIN.EMP_NO, MAIN.EMP_NAME, MAIN.MANAGER_ID,
	NVL((
		SELECT 
			SUB.EMP_NAME
		FROM
...
-- 2) 1번 SELECT문을 인라인 뷰로 사용하여 상위 3개 부서만 조회
SELECT 
	ROWNUM, "부서 코드", "부서명", "급여 평균"
FROM(
	SELECT 
		NVL(E.DEPT_CODE, '없음') "부서 코드",
		NVL(D.DEPT_TITLE,'없음') "부서명",
...
/* [ROWNUM 사용 시 주의사항]
 * 
 * ROWNUM이 WHERE절에 사용되는 경우 항상 1부터 연속적인 범위가 지정되어야함
 * 
...
WHERE 
	E.SALARY = (
		...
		FROM
			EMPLOYEE SUB
		WHERE 
			TO_CHAR(SUB.HIRE_DATE, 'YYYY') = '2010'
	);
...
WHERE
  EXISTS (
    ...
    AND SUB.DEPT_CODE = E.DEPT_CODE
    AND SUB.JOB_CODE = E.JOB_CODE
  );
