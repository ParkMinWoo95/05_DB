/* SELECT
 * - 지정된 테이블에서 원하는 데이터가 존재하는 행, 열을 선택해서 조회하는 SQL
 ...
 * 	 조회 결과(RESULT SET)에서 몯ㄴ 행에 산술 연산이
 * 	 적용된 컬럼 값이 조회된다
 */
...
-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 입사일, 현재까지 근무 일수, 연차 조회
SELECT
	EMP_ID, 
	EMP_NAME,
	HIRE_DATE,
	FLOOR(CURRENT_DATE - HIRE_DATE), -- 내림 처리
	CEIL((CURRENT_DATE - HIRE_DATE) / 365) -- 내림 처리
FROM EMPLOYEE;
...
-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 입사일, 근무 일수, 연차 조회
-- 단, 별칭 꼭 지정
SELECT
	EMP_ID AS 사번, -- AS 별칭
	EMP_NAME 이름, -- AS 생략
...
-- EMPLOYEE 테이블에서 모든 사원의 사번, 이룸, 급여(원), 연봉(급여*12) 조회
-- 단, 컬럼명은 모두 별칭 적용
SELECT
	EMP_ID 사번, -- AS 별칭
	EMP_NAME 이름,
 ...
 * - CHAR, VATCHAR2 타입(문자열) : 'AB', '가나다' ('' 홑따옴표)
 * 
 * SELECT절에 리터럴을 작성하면 조회 결과(RESULT SET) 모든 행에 리터럴이 추가된다
 */
SELECT
	SALARY,
	'원',
...
/* DISTINCT(별개의, 전혀 다른)
 * - 조회 결과 집합(RESULT SET)에서 DISTICT가 지정된 컬럼에 중복된 값이 존재할 경우
 * 	 중복을 제거하고 한 번만 표시할 때 사용
 * 
 * (중복된 데이터를 가진 행을 제거)
 */

-- EMPLOYEE 테이블에서 모든 사원의 부서 코드(DEPT_CODE) 조회
SELECT
	DEPT_CODE
FROM
	EMPLOYEE; -- 23행 조회
 ...
 * - NULL은 비교대상이 없기때문에 =, != 등의 비교연산 결과가 무조건 FALSE
 */
...
TO_DATE('2010.01.01', 'YYYY.MM.DD') AND TO_DATE('2019.12.31', 'YYYY.MM.DD');
...
FROM EMPLOYEE
WHERE
	DEPT_CODE = 'D5' OR DEPT_CODE = 'D6' OR DEPT_CODE = 'D9';
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE 
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN ('D5', 'D6', 'D9') OR DEPT_CODE IS NULL; -- 11행(NULL)
 ...
 * 	-> 지정된 특수문자 뒤 '딱 한 글자'를 와일드 카드가 아닌
 * 		 단순 문자열로 인식시키는 옵션
 * 
 * - 작성법
 ...
 * 
 * ORDER BY 절
 * - SELECT의 조회 결과 집합(RESULT SET)을 원하는 순서로
 * 	 정렬할 때 사용하는 구문
 * 
 * - 작성법
 * 
 ...
 * - ORDER BY절보다 먼저 해석되는 SELECT절 별칭을 ORDER BY절에서 인식할 수 있다
 * 
 * ?? 그럼 SELECT절 보다 먼저 해석되는 WHERE절에서 별칭 사용이 가능할까?
 * 	-> 불가능
 */
...
/* 컬럼 순서를 이용하여 정렬하기
 * - SELECT절이 해석되어 조회하려면 컬럼이 지정되면서 컬럼의 순서도 같이 지정된다
...
SELECT 
	...
	JOB_CODE AS 직급코드
FROM EMPLOYEE
ORDER BY 부서코드 ASC, 직급코드 DESC, 성명 ASC;
